...
import {
  IBooks,
  IBooksResponse,
} from '../../../books-search/shared/interfaces/books-response.interface';
...
export class BooksSearchService {
  ...
  getFilteredBooks() {
    ...
    for (const key in searchParamsQuery) {
      if (
        searchParamsQuery[key] !== undefined &&
        searchParamsQuery[key] !== null &&
        searchParamsQuery[key] !== ''
      ) {
        if (queryString !== '') {
    ...
    for (const key in searchParamsFilter) {
      if (
        searchParamsFilter[key] !== undefined &&
        searchParamsFilter[key] !== null &&
        searchParamsFilter[key] !== ''
      ) {
