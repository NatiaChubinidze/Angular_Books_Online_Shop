...
export class AllBooksReadComponent implements OnInit {
  p: number = 1;
  ...
  ngOnInit(): void {
    ...
    if(this._adminService.readBooksList){
    this._adminService.readBooksList.forEach(item=>{
      if(this._adminService.readBooksList.indexOf(item)===0){
